/*
* CapibaraBot es una creación de AleV
* CapibaraBot no tiene ningun fin de lucro
* AleV se reserva todos los derechos de autor
* © 2021 CapibaraBot, INC.
 */
...
async function starts() {
	...
	console.log('>', '[',color('INFO','blue'),']','Gracias por usar CAPIBARABOT...')
	turbo.on('qr', () => {
...
//Welkom
			...
			teks += `@${num.split('@')[0]}\n`
			teks += `${mdata.subject}\n`
		    teks += `Me complace darles la bienvenida a este grupo de WhatsApp.\n`
			teks += `Estoy encantado de comenzar una fructífera relación y colaboración entre toda la comunidad.\n`
            teks += `Les recuerdo de manera formal, el cumplimiento de las condiciones para mantener una comunicación equilibrada, con los fines para lo cual se creó este grupo, fomentando el respeto y la colaboración.\n`
			...
			teks = `Hoy no es un día para recordar que te vas y ya nunca más volverás.\n`
			teks += `Hoy en un día para recordar todos los buenos momentos que hemos pasado a tu lado.\n`
       　 teks += `Adiós.\n`
            teks += ` @${num.split('@')[0]} 🐈💨\n`
			let buff = await getBuffer(ppimg)
			turbo.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			...
			teks = `*ERES EL NUEVO ADMINISTRADOR 😎*
...
//
		...
		mess = {
			wait: 'Aguarda un momento, estamos en proceso de creación, 90%.',
			success: 'Comando con  éxito',
			wrongFormat: 'Formato equivocado, por favor corrígelo.',
			error: {
				stick: 'Lo siento, no se pudo convertir a sticker.',
				Iv: 'Lo siento, tu link es inválido'
			},
			only: {
				group: 'Este comando solo puede ser utilizado en grupos, intentalo de nuevo en uno.',
				admin: 'Lo siento, este comando solo puede ser utilizado por administradores.',
				Badmin: 'Necesito los permisos de administrador para ejecutar el comando.'
			}
		}
		const isUrl = (url) => {
...
reply(`Los link no están permitidos${sender.split("@")[0]}`)
setTimeout( () => {
...
Buenos dias, tardes, noches, ${pushname}. Bienvenido al menú "Welcome menú"
════════════════════════
AleV: https://www.youtube.com/channel/UCi_aqZjNb8qgSZ0Kbk-M8Lg
════════════════════════
...
✓ Chats : ${giid.length}
✓ Grupos : ${totalchat.length - giid.length}
✓ Total de chats :${totalchat.length}
✓ Velocidad : ${sepid.toFixed(4)} 𝐬𝐞𝐠𝐮𝐧𝐝𝐨𝐬
✓ Tiempo : ${runtem}
✓ Prefix : 「 ${prefix} 」
...
`
    ...
    case prefix+ 'off':
            ...
            alasan = anuu
            fakestatus(' APAGADO. INTENTE DE NUEVO MÁS TARDE')
            break
    case prefix+ 'status':
...
if (pesan.length > 300) return turbo.sendMessage(from, 'El informe es demasiado largo, máximo 300 caracteres', msgType.text, {quoted: sam})
...
break
     case prefix+ 'get':
      if(!q) return reply('Preciso de un enlace o hipervínculo para ejecutar el comando.')
			.then(res => res.json())
			qweryna = args.join(' ')
...
case prefix+ 'avatar':
		...
		case prefix+ 'waifu':
			...
			turbo.sendMessage(from, nyed, image, { caption: 'AleV', quoted: mek })
			.catch(err => {
				return('Ocurrió un error, intentalo de nuevo...')
			})
			break
...
break
	...
	case prefix+ 'husbu':
			...
			turbo.sendMessage(from, nye, image, { caption: '❮ ❮ AleV ❯❯', quoted: mek })
			.catch(err => {
				return('Ocurrió un error, intentalo de nuevo...')
			})
			break
case prefix+ 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						turbo.sendMessage(from,`Ha sido activado antilink, si mandan algún link serán eliminados`, text)
...
case prefix+ 'pinterest':
			...
			turbo.sendMessage(from, nye, image, { caption: 'AleV', quoted: mek })
			.catch(err => {
				return('Preciso de un enlace, hipervínculo o nombre para ejecutar el comando.')
			})
			break
    case prefix+ 'anime':
            reply(mess.wait)
            ...
            turbo.sendMessage(from,media,image,{quoted:mek,caption:'AleV'})
            }
            )
            .catch((error) => {
            console.log(error); 
...
case prefix+ 'getbio':
	   ...
	   case prefix+ 'getpic':
		...
		turbo.sendMessage(from, thumb, MessageType.image, {caption: 'AleV'})
	        }
		break
case prefix+ 'setbio':
	        if (!isMe) return reply('Esté comando solo puede usado por mi')
...
case prefix+  'stats':
                ...
                await wa.sendFakeStatus(from, texxt, "AleV")
                break
case prefix+ 'block':
...
case prefix+ 'gtav':
  ...
  if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
   ...
   turbo.sendMessage(from, hehe, image, {quoted:mek})
	...
	reply('Preciso de un enlace, hipervínculo o nombre para ejecutar el comando.')
  }
  break
case prefix+ 'wanted':
  var imgbb = require('imgbb-uploader')
...
case prefix+ 'infoall':
					...
					break
case prefix+ 'emoji':
			if (!q) return fakegroup('Preciso de un enlace, hipervínculo, nombre o emoji para ejecutar el comando.')
			qes = args.join(' ')
...
//CONVERTER
    ...
    case prefix+ 'tovideo':
            ...
            sendMediaURL(from,res.result,'AleV')
            })
            }else {
 ...
 case prefix+ 'reglas':
 case prefix+ 'rules':
        her = `*REGLAS PARÁ USAR EL CAPIBARABOT*
...
case prefix+ 'ssweb':
					...
					if (args.length < 1) return reply('Para ejecutar el comando es necesario un enlace y/o hipervínculo.')
					teks = `${body.slice(7)}`
					reply(ind.wait())
...
//Upload Story WeA
    case prefix+ 'upswteks':
            ...
            fakegroup(`Se subió con éxito el estado con el texto ${q}`)
            break
